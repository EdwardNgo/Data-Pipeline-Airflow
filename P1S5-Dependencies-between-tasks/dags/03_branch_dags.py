...
def _pick_erp_system(**context):
    if context["execution_date"] < ERP_CHANGE_DATE:
        ...
        return "fetch_sales_new"# return id of downstream task not the function
...
with DAG(
    ...
    # Neu viet nhu tren kia thi cai pipeline cuoi se bi skip vi theo trigger rule thong thuong la tat ca upstream task phai ok moi thuc hien ma o tren do chi co 2 cai -> ta co the thay trigger rule nhung khong clean cho lam thuong nguoi ta se join ket qua
    start >> [pick_erp_system, fetch_weather]
    pick_erp_system >> [fetch_sales_old, fetch_sales_new]
    fetch_sales_old >> clean_sales_old
    fetch_sales_new >> clean_sales_new
    [clean_sales_old, clean_sales_new] >> join_erp # day join day ne
    fetch_weather >> clean_weather
